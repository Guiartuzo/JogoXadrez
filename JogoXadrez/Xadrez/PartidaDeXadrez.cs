...
namespace Xadrez
{
    class PartidaDeXadrez
    {
        ...
        public void RealizaJogada(Posicao origem, Posicao destino)
        {
            ...
            if (p is Peao)
            {
                if ((p.CorPeca == Cor.Branca && destino.Linha == 0) || (p.CorPeca == Cor.Preta && destino.Linha == 7))
                {
                    p = Tab.RetirarPeca(destino);
                    PecasEmJogo.Remove(p);
                    Peca dama = new Dama(Tab, p.CorPeca);
            ...
            foreach (Peca x in GetPecasEmJogo(cor))
            {
                ...
                for (int i = 0; i < Tab.Linhas; i++)
                {
                    for (int j = 0; j < Tab.Colunas; j++)
                    {
                        if (mat[i, j])
                        {
                            Posicao origem = x.PosicaoTabuleiro;
        ...
        {
            //Posicionando peças brancas no tabuleiro
            ...
            //Posicionando peças pretas no tabuleiro
